...
const getTorneoNombre = async (req, res) => {
    ...
    if(typeof nombre === 'string') {
        const response = await database.query('SELECT * FROM torneos WHERE nombre = $1', [nombre]);
...
const getTorneoIni = async (req, res) => {
    ...
    if(typeof fecha_ini === 'date') {
        const response = await database.query('SELECT * FROM jugadores WHERE fecha_ini = $1', [fecha_ini]);
        ...
        res.status(400).json({error: 'Torneo no encontrado bajo esa fecha inicial'});
    }
};
...
const getTorneoFin = async (req, res) => {
    ...
    if((typeof fecha_fin === 'date')) {
        const response = await database.query('SELECT * FROM torneos WHERE fecha_fin = $1', [fecha_fin]);
        ...
        res.status(400).json({error: 'Torneo no encontrado bajo esa fecha final'});
    }
};
